...
class _StatsTabState extends State<StatsTab> {
  ...
  // Uses time-aggregated data based on selected range
  List<double> _getSparklineData(String metric) {
    if (_allSwings.isEmpty) return [];

    // Create chart data with time aggregation
    ...
    final dataPoints = chartData.getDataPoints(metric);
    if (dataPoints.isEmpty) return [];

    // Extract values from aggregated points
...
class _EnlargedChartSheetState extends State<_EnlargedChartSheet> {
            ...
            Padding(
              ...
              child: Wrap(
                ...
                children: [
                  ...
                  _infoChip(
                      isDark, 'Total ${widget.chartData.totalShots} shots'),
                  _infoChip(isDark, _getTimeRangeLabel(widget.chartData.range)),
                ],
              ),
            ),
            SizedBox(
              ...
              child: Padding(
                ...
                child: InteractiveLineChart(
                  ...
                  yUnit: unit,
                  xAxisLabel: _getXAxisLabel(widget.chartData.range),
                  minY: minY,
                  maxY: maxY,
  ...
  String _getTimeRangeLabel(TimeRange range) {
    return switch (range) {
      TimeRange.daily => 'Individual swings',
      TimeRange.weekly => 'Last 7 days',
      TimeRange.monthly => 'Last 30 days',
      TimeRange.yearly => 'Last 12 months',
      TimeRange.all => 'All time (monthly)',
    };
  }

  String _getXAxisLabel(TimeRange range) {
    return switch (range) {
      TimeRange.daily => 'Swing #',
      TimeRange.weekly => 'Day',
      TimeRange.monthly => 'Day',
      TimeRange.yearly => 'Month',
      TimeRange.all => 'Month',
    };
  }
