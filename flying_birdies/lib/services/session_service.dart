...
class SessionService implements ISessionService {
  ...
  SessionSummary _calculateSessionSummary(
    ...
    double avgForceN = 0;
    double maxForceN = 0;
    double avgSwingForceN = 0;
    double maxSwingForceN = 0;
    ...
    if (swings.isNotEmpty) {
      ...
      // Impact force metrics (from est_force_n)
      avgForceN = swings.map((s) => s.estForceN).reduce((a, b) => a + b) /
          swings.length;
      maxForceN =
          swings.map((s) => s.estForceN).reduce((a, b) => a > b ? a : b);

      // Swing force metrics (from impact_severity)
      avgSwingForceN =
          swings.map((s) => s.impactSeverity).reduce((a, b) => a + b) /
              swings.length;
      maxSwingForceN =
          swings.map((s) => s.impactSeverity).reduce((a, b) => a > b ? a : b);
    ...
    return SessionSummary(
      ...
      avgForceN: avgForceN,
      maxForceN: maxForceN,
      avgSwingForceN: avgSwingForceN,
      maxSwingForceN: maxSwingForceN,
