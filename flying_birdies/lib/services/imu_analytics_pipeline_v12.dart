...
class ImuConfig {
  ...
  // FFT validation threshold (adjusted for fftea library scaling)
  // Python uses 35.0 with NumPy's FFT
  // fftea produces power 2.97x higher than NumPy, so threshold = 35 * 2.97 ≈ 104
  static const double micPerGyroThreshold = 25.0;
...
class SwingAnalyzer {
  ...
  SwingMetrics? processReading(ImuReading reading) {
    ...
    if (swing != null && swing.isValidSwing) {
      ...
      // Even if no swing detected, update index to avoid re-analyzing same data
